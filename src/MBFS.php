...
namespace MBFoundation\PhpMbfsApi;
...
class MBFS {
	...
	public function cat ($hash,$acckey) {
		$ip = $this->gatewayIP;
		$port = $this->gatewayPort;
//		return $this->curl("http://$ip:$port/ipfs/$hash");
		return $this->curl("http://$ip:$port/api/v0/cat?arg=$hash&acckey=$acckey");

	}

	public function get ($hash,$acckey) {
		$ip = $this->gatewayIP;
		$port = $this->gatewayPort;
		return $this->curl("http://$ip:$port/api/v0/get?arg=$hash&acckey=$acckey");
//curl "http://localhost:5001/api/v0/get?arg=<ipfs-path>&output=/&archive=false&compress=false&compression-level=-1"

	}

	public function add ($content, $acckey) {
		$ip = $this->gatewayIP;
		$port = $this->gatewayApiPort;

//		echo "ip:$ip----port:$port----acckey:[$acckey]----content:$content";
		$req = $this->curl("http://$ip:$port/api/v0/add?stream-channels=true&acckey=$acckey&output=", $content);
//		$req = $this->curl("http://$ip:$port/api/v0/add?acckey=$acckey", $content);
